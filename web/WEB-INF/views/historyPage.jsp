...
<div class="layui-layout layui-layout-admin">
            ...
            <ul class="layui-nav layui-layout-right">
                ...
                <li class="layui-nav-item" lay-unselect="" lay-filter="me">
                    <a href="javascript:;"><img src="${pageContext.request.contextPath}/static/头像.jpg"
                                                class="layui-nav-img" id="userName">${sessionScope.user.id}|${sessionScope.user.name}</a>
    ...
    <div class="layui-body">
        ...
        <table class="layui-table"
               lay-data="{height: 'full-200', cellMinWidth: 80,url:'${pageContext.request.contextPath}/userHistoryList', id:'test', limit: 10}"
               lay-filter="historyData">
            <thead>
            <tr>
                ...
                <th lay-data="{fixed: 'right', title:'操作', align:'center', toolbar: '#operation', width:130}">操作
                </th>
            </tr>
            </thead>
        </table>
    ...
    <script type="text/html" id="operation">
        {{#  if(d.id == '${sessionScope.user.id}'){ }}
        <a class='layui-btn layui-btn-normal layui-btn-xs' style="background: #FF5722" lay-event='del'>删除记录</a>
        {{#  } }}
    </script>
    ...
    <script>
        ...
        layui.use(['element', 'layer', 'form', 'upload', 'table'], function () {
            ...
            table.on('tool(historyData)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
                var data = obj.data; //获得当前行数据
                var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
                var tr = obj.tr; //获得当前行 tr 的 DOM 对象（如果有的话）

                if (layEvent === 'del') { //删除
                    layer.confirm('是否删除该条历史订单', function (index) {
